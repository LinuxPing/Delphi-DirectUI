...
//构造函数
constructor TDuMemoryDC.Create(DC: HDC);
begin
  inherited Create;
  //创建和目标DC兼容的内存DC
...
//构造函数
constructor TDuMemoryDC.Create(DC: HDC; const ABoundsRect: TRect);
begin
  inherited Create;
  //创建和目标DC兼容的内存DC
  m_hMemDC := CreateCompatibleDC(DC);
...
//析构函数
destructor TDuMemoryDC.Destroy;
begin
  if m_hMemDC <> 0 then
  begin
...
//设置绘制区域
procedure TDuMemoryDC.SetBounds(DC:HDC; const ABoundsRect: TRect);
begin
  //检查区域是否为空
...
//将内存DC绘制到目标DC
procedure TDuMemoryDC.Blt(DC:HDC);
begin
...
//将内存DC绘制到目标DC的指定区域上(要保证AInvalidateRect和m_rcBounds在一个坐标系上)
procedure TDuMemoryDC.Blt(DC: HDC; AInvalidateRect: TRect);
begin
